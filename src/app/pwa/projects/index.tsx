  ...
  return (
    <div className='w-full'>
      <div className='flex flex-row items-center font-normal w-full py-1'>
        ...
        <div className={navBarButton}>
          <BsArrowClockwise className='place-self-center' />
        </div>
        ...
        <button
          ...
          onClick={() => setFilter(nameRef.current?.value ?? '')}
        >
          <BsSearch />
        </button>
        <button
          ...
          onClick={async () => {
            ...
            if (response instanceof HTMLElement) {
              const name =
                (response.querySelector('input[name=name]') as HTMLInputElement)
                  ?.value ?? '';
              const path =
                (response.querySelector('input[name=path]') as HTMLInputElement)
                  ?.value ?? '';
        ...
        {groups.map((group) => (
          ...
          >
            <button className='w-full text-center'>
              ...
              <div className='text-sm font-semibold truncate'>{group}</div>
            </button>
          </div>
        ))}
        {projects
...
export default function Projects() {
  ...
  const decryptProject = async (project: ProjectType) => {
    if (project.name && isEncrypted(project as unknown as EncryptedData)) {
      let error;
  ...
  const setDecryptedProjects = useCallback(async (projects: ProjectType[]) => {
