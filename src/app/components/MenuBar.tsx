...
export default function MenuBar({ model, onCreate }: Readonly<MenuBarProps>) {
  ...
  return (
    ...
    >
      <div className='flex items-center space-x-4 h-full w-full bg-zinc-100 text-zinc-900'>
        ...
        {model.map((item) => {
          ...
          if (item.separator) {
            return (
              <div
                key='separator'
                className='w-px h-full bg-zinc-300 mx-2'
              ></div>
            );
          }
          ...
          return (
            <div className='relative' key={item.label}>
              ...
              >
                {item.icon}{' '}
                {item.icon ? <div className='mr-2'></div> : undefined}
                {item.label}
              </button>
